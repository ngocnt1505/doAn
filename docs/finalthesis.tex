...
FR-3 & Countdown shows 3--2--1--Ready, one per second, then play begins on its own. \emph{Check:} play starts with no extra input after ``Ready''. & \texttt{reducer.ts:190}, \texttt{constants.ts:12} \\ \hline
FR-28 & Restart returns straight to a fresh countdown, keeping the name. \emph{Check:} ``Restart'' begins a new run. & \texttt{reducer.ts:61} \\ \hline

\multicolumn{3}{|l|}{\textbf{Gameplay Environment}} \\ \hline
BR-12/28 & Fixed zones along $x$ (house / strip / yard); the house front spans the depth $z$. \emph{Check:} monsters cross the green yard toward the house. & \texttt{constants.ts:29} \\ \hline
...
FR-8 & Speed derived from a per-class field-crossing time (easy fastest). \emph{Check:} easy reaches the house before hard. & \texttt{movementSystem.ts:15} \\ \hline
...
BR-27--30 & Enemies weave on a smooth sine path, spread across depth. \emph{Check:} enemies do not funnel into one line. & \texttt{movementSystem.ts:33} \\ \hline
...
BR-91--93 & Clearing wave 1/2 unlocks the next tier. \emph{Check:} Medium is selectable after wave 1. & \texttt{weapons.ts:57}, \texttt{reducer.ts:24} \\ \hline

\multicolumn{3}{|l|}{\textbf{Shooting and Damage}} \\ \hline
FR-15 & A left-click on the yard drops the marker and fires. \emph{Check:} an ``X'' marks the click and a shot launches. & \texttt{GameCanvas.tsx:111} \\ \hline
FR-16 / BR-130--132 & A shot is ignored while the weapon reloads. \emph{Check:} rapid clicks cannot beat the reload. & \texttt{reducer.ts:114} \\ \hline
...
BR-23 / TR-4 & The first enemy of a wave arrives 3\,s after ``Ready''. \emph{Check:} a short pause precedes the first spawn. & \texttt{spawnSystem.ts:19} \\ \hline
...
FR-23 & A wave clears once its whole roster has spawned and no enemy remains --- decided in the reducer. \emph{Check:} reward/victory appears only after the last monster is gone. & \texttt{reducer.ts:190}, \texttt{waves.ts:14} \\ \hline
...
FR-29 / BR-104--105 & Defeat the instant a moving enemy reaches the house line (positional, not HP). \emph{Check:} one breach ends the game. & \texttt{reducer.ts:88} \\ \hline
FR-31 & Victory once the third wave is fully cleared. \emph{Check:} clearing wave 3 shows the win screen. & \texttt{reducer.ts:190} \\ \hline

\multicolumn{3}{|l|}{\textbf{Heads-Up Display}} \\ \hline
HUD & The HUD shows wave, elapsed time and active weapon, and freezes with the simulation. \emph{Check:} the timer stops while paused. & \texttt{HUD.tsx} \\ \hline
...
Lighting & Ambient light plus one directional ``sun'' with soft shadow maps. \emph{Check:} enemies cast shadows on the yard. & \texttt{lighting.ts:7}, \texttt{threeSetup.ts:23} \\ \hline
...
FR-39 & Bullets are removed after flight plus a short linger. \emph{Check:} projectiles vanish soon after landing. & \texttt{bulletSystem.ts:28} \\ \hline

\multicolumn{3}{|l|}{\textbf{Leaderboard and Ranking}} \\ \hline
Schema & A finished run stores name, time, wave, won and a timestamp (one row). \emph{Check:} each completed run adds a row. & \texttt{db.ts:30} \\ \hline
...
Validation & The server rejects malformed runs (name; time $>0$; wave 1--3; won boolean). \emph{Check:} a bad POST returns 400. & \texttt{route.ts:31} \\ \hline
